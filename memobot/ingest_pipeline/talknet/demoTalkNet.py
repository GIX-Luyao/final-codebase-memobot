...
parser.add_argument('--confidenceThreshold',   type=float, default=0.0, help='Confidence threshold for active speaker detection (lower = more permissive, default: 0.0)')
...
if os.path.isfile(args.pretrainModel) == False: # Download the pretrained model
    ...
    gdown.download(id=Link, output=args.pretrainModel, quiet=False)
...
if args.evalCol == True:
	...
	if os.path.isdir(args.videoFolder + '/col_labels') == False: # Download label
		...
		gdown.download(id=link, output=args.videoFolder + '/col_labels.tar.gz', quiet=False)
