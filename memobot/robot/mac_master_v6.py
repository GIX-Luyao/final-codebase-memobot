...
async def main():
    ...
    # Start realtime agent without waiting for recognition (user detected on speech via VAD)
    if original_server.use_realtime:
        key = os.environ.get("OPENAI_API_KEY")
        if key:
            print("[System] Starting Realtime Agent (user will be detected on first speech)...")
            original_server.agent_instance = original_server.ServerRealtimeAgent(key, user_name=None, person_id=None)
            asyncio.create_task(original_server.agent_instance.run())

    def _run_recognition_on_frame(frame_copy):
        """Run face recognition in a thread and update current speaker."""
        global _current_speaker
        person = recognize_user_from_frame(frame_copy)
