...
# Network latency (seconds) to shift audio so it aligns with delayed video in the ingest pipeline clip.
# Increase if audio leads video; decrease if video leads audio.
...
class ClipRecorder:
    ...
    def _finalize_clip(self, frames, user_chunks, tts_chunks, clip_start):
        ...
        # Shift audio by network_latency_delay so it aligns with video (video typically arrives delayed).
        latency_offset = int(self.network_latency_delay * self.sample_rate)
        for t, data in user_chunks:
            offset = int((t - clip_start) * self.sample_rate) + latency_offset
        ...
        print(f"✅ Clip Saved: {final_filename} (stereo: L=user, R=TTS, audio delay={self.network_latency_delay}s)")
