...
resources = {
    ...
    "thread_code_receiver": None,
    "stop_code_thread_event": None,
}
...
def run_received_code(code_str):
    """Execute code string in a subprocess; capture and print STDOUT (and STDERR)."""
    try:
        fd, path = tempfile.mkstemp(suffix=".py", prefix="robot_code_")
        try:
            to_write = code_str.encode("utf-8") if isinstance(code_str, type(u"")) else code_str
            ...
            if proc.returncode != 0:
                print "[Code exit code: %s]" % proc.returncode
        finally:
            if fd is not None:
                try:
                    os.close(fd)
        ...
        print "[Code execution error] %s" % e


def thread_code_receiver(mac_ip, port, stop_event):
    """Connect to mac code channel; receive length-prefixed code, print and execute in subprocess."""
    while not stop_event.is_set():
        try:
            ...
            print "[Code] Connected to %s:%s (executed code channel)" % (mac_ip, port)
            while not stop_event.is_set():
                ...
                if msg_len <= 0 or msg_len > 10 * 1024 * 1024:
                    print "[Code] Invalid length %s, dropping." % msg_len
                    break
                ...
                print "\n" + "=" * 60
                print "RECEIVED CODE FROM MAC"
                print "=" * 60
                print code_str
                print "=" * 60 + "\n"
                run_received_code(code_str)
        except (BrokenPipeError, ConnectionResetError, OSError) as e:
            print "[Code] Disconnected: %s" % e
        except Exception as e:
...
def start_services(args, robot_ip):
    ...
    # 1b. Start Code Receiver (connect to mac_master code channel)
    stop_code = threading.Event()
    t_code = threading.Thread(
        target=thread_code_receiver,
        args=(args.mac_ip, args.port_code_rx, stop_code),
    )
    t_code.daemon = True
    t_code.start()
    resources["thread_code_receiver"] = t_code
    resources["stop_code_thread_event"] = stop_code
...
def stop_services():
    ...
    if resources["stop_code_thread_event"]: resources["stop_code_thread_event"].set()
...
def main():
    ...
    parser.add_argument("--port-code-rx", type=int, default=50009, help="Port to receive executed code from mac_master (length-prefixed)")
